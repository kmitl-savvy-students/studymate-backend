...
public class SdmTeachtableSubjectReview
{
        ...
        while (query.Next())
        {
            ...
            if (query.ToInt(1) > 0)
            {
                ...
                if (teachtableSubject != null)
                {
                    ...
                    if (subject != null) subjectNameEn = subject.subject_ename; // ดึง subject_name_en
                }
            }
    ...
    public static void Insert(TeachtableSubjectReview review)
    {
        try
        {
            if (review.teachtable_subject == null || review.teachtable_subject.id == 0) throw new Exception("teachtable_subject is null or has invalid id.");

            if (string.IsNullOrEmpty(review.user_id)) throw new Exception("User is null or has invalid id.");
    ...
    public static List<TeachtableSubjectReview> GetBySubject(string subjectId)
    {
        try
        {
            ...
            if (subjectResult.Count == 0) return new List<TeachtableSubjectReview>(); // คืนค่าเป็นลิสต์ว่างถ้าไม่พบข้อมูล
            ...
            foreach (var subject in subjectResult)
            {
                ...
                if (reviewResult.Count > 0) allReviews.AddRange(reviewResult); // รวมข้อมูลทั้งหมดในลิสต์เดียว
            }
    ...
    public static TeachtableSubjectReview? GetBySubjectAndStudent(string subjectId, string studentId)
    {
        try
        {
            ...
            if (subjectResult.Count == 0) return null; // ไม่มี teachtable_subject_id ที่เกี่ยวข้อง
            ...
            foreach (var teachtableSubject in subjectResult)
            {
                ...
                var reviewResult = ProcessQuery(selectReview, true);
                if (reviewResult.Count > 0) return reviewResult[0]; // คืนค่าหากมีรีวิวที่ตรงกัน
            }
    ...
    public static void Delete(string subjectId, string studentId)
    {
        try
        {
            ...
            if (subjectResult.Count == 0) throw new Exception("TeachtableSubject not found.");
    ...
    public static void CreateReview(string studentId, int year, int term, string subjectId, string review, float rating)
    {
        ...
        try
        {
            ...
            if (teachableSubject == null || teachableSubject.id == 0) throw new Exception("TeachtableSubject is null or has invalid id.");
            ...
            if (user == null || string.IsNullOrEmpty(user.Id)) throw new Exception("User is null or has invalid id.");
            ...
            foreach (var faculty in jsonArray)
            {
                ...
                foreach (var teachTable in teachTableArray)
                {
                    ...
                    foreach (var data in dataArray)
                    {
                        ...
                        if (!string.IsNullOrEmpty(subjectId) && !allSubjects.Contains(subjectId)) allSubjects.Add(subjectId); // เพิ่ม subject_id ถ้ายังไม่มีใน List
                    }
                }
            }
        }
    ...
    {
        var currentYear = DateTime.Now.Year + 543; // คำนวณปี พ.ศ.
        int[] terms = { 3, 2, 1 };
            ...
            foreach (var currentTerm in terms)
            {
                ...
                if (responseFaculty.IsSuccessStatusCode)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(dataOfFaculty) && dataOfFaculty != "[]")
                    {
                        ...
                        foreach (var subjectId in allSubjectsOfFaculty) Console.WriteLine($"- {subjectId}");
                        Console.WriteLine("[GetAllSubjectInFacultyAndGenedTest] AllSubjectsOfGened:");
                        foreach (var subjectId in allSubjectsOfGened) Console.WriteLine($"- {subjectId}");
                        Console.WriteLine("[GetAllSubjectInFacultyAndGenedTest] AllSubjects:");
                        foreach (var subjectId in allSubjects) Console.WriteLine($"- {subjectId}");
    ...
    public static List<TeachtableSubjectReview> GetReviewsBySubjects(List<string> subjectIds)
    {
        ...
        foreach (var subjectId in subjectIds)
        {
            ...
            if (reviews.Count > 0) allReviews.AddRange(reviews); // รวมรีวิวทั้งหมดในลิสต์เดียว
        }
